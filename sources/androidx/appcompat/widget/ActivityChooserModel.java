...
class ActivityChooserModel extends DataSetObservable {
        ...
        public void sort(Intent intent, List<ActivityResolveInfo> list, List<HistoricalRecord> list2) {
            ...
            for (int size2 = list2.size() - 1; size2 >= 0; size2--) {
                ...
                if (activityResolveInfo2 != null) {
                    activityResolveInfo2.weight += historicalRecord.weight * f11;
                    f11 *= 0.95f;
                }
            }
    ...
    public static final class HistoricalRecord {
        ...
        public int hashCode() {
            ...
            long j11 = this.time;
            return ((((hashCode + 31) * 31) + ((int) (j11 ^ (j11 >>> 32)))) * 31) + Float.floatToIntBits(this.weight);
        }
    ...
    private boolean loadActivitiesIfNeeded() {
        ...
        for (int i11 = 0; i11 < size; i11++) {
            this.mActivities.add(new ActivityResolveInfo(queryIntentActivities.get(i11)));
        }
        return true;
    }
        ...
        try {
            ...
            try {
                try {
                    ...
                    Log.e(LOG_TAG, "Error reading historical recrod file: " + this.mHistoryFileName, e12);
                    if (openFileInput == null) {
                        return;
                    }
                    openFileInput.close();
    ...
    public Intent chooseActivity(int i11) {
        synchronized (this.mInstanceLock) {
            try {
                ...
                ActivityInfo activityInfo = this.mActivities.get(i11).resolveInfo.activityInfo;
    ...
    public ResolveInfo getActivity(int i11) {
        ResolveInfo resolveInfo;
        synchronized (this.mInstanceLock) {
            ensureConsistentState();
            resolveInfo = this.mActivities.get(i11).resolveInfo;
        }
        return resolveInfo;
    }
    ...
    public int getActivityIndex(ResolveInfo resolveInfo) {
        synchronized (this.mInstanceLock) {
            try {
                ...
                int size = list.size();
                for (int i11 = 0; i11 < size; i11++) {
                    if (list.get(i11).resolveInfo == resolveInfo) {
                        return i11;
                    }
                }
                return -1;
    ...
    public void setDefaultActivity(int i11) {
        synchronized (this.mInstanceLock) {
            try {
                ensureConsistentState();
                ActivityResolveInfo activityResolveInfo = this.mActivities.get(i11);
                ActivityResolveInfo activityResolveInfo2 = this.mActivities.get(0);
                float f11 = activityResolveInfo2 != null ? (activityResolveInfo2.weight - activityResolveInfo.weight) + 5.0f : 1.0f;
                ActivityInfo activityInfo = activityResolveInfo.resolveInfo.activityInfo;
                addHistoricalRecord(new HistoricalRecord(new ComponentName(activityInfo.packageName, activityInfo.name), System.currentTimeMillis(), f11));
            } catch (Throwable th2) {
                throw th2;
            }
        }
    ...
    public void setHistoryMaxSize(int i11) {
        synchronized (this.mInstanceLock) {
            try {
                if (this.mHistoryMaxSize == i11) {
                    return;
                }
                this.mHistoryMaxSize = i11;
                pruneExcessiveHistoricalRecordsIfNeeded();
                if (sortActivitiesIfNeeded()) {
                    notifyChanged();
                }
