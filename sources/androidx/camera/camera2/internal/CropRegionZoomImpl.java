...
final class CropRegionZoomImpl implements ZoomControl.ZoomImpl {
    ...
    @NonNull
    private static Rect getCropRectByRatio(@NonNull Rect rect, float f11) {
    ...
    public float getMaxZoom() {
        Float f11 = (Float) this.mCameraCharacteristics.get(CameraCharacteristics.SCALER_AVAILABLE_MAX_DIGITAL_ZOOM);
        if (f11 == null) {
            return 1.0f;
        }
    ...
    @Override // androidx.camera.camera2.internal.ZoomControl.ZoomImpl
    public void setZoomRatio(float f11, @NonNull CallbackToFutureAdapter.Completer<Void> completer) {
        this.mCurrentCropRect = getCropRectByRatio(getSensorRect(), f11);
