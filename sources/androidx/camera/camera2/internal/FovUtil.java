...
public class FovUtil {
    ...
    public static int focalLengthToViewAngleDegrees(float f11, float f12) {
        Preconditions.checkArgument(f11 > 0.0f, "Focal length should be positive.");
        Preconditions.checkArgument(f12 > 0.0f, "Sensor length should be positive.");
    ...
    public static int getDeviceDefaultViewAngleDegrees(@NonNull CameraManagerCompat cameraManagerCompat, int i11) {
        try {
            for (String str : cameraManagerCompat.getCameraIdList()) {
                ...
                if (num.intValue() == LensFacingUtil.getLensFacingInt(i11)) {
