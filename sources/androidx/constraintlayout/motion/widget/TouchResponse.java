...
class TouchResponse {
    ...
    public void processTouchEvent(MotionEvent motionEvent, MotionLayout.MotionTracker motionTracker, int i11, MotionScene motionScene) {
        char c11;
        int i12;
        ?? r23;
        float f11;
        ...
        if (this.mIsRotateMode) {
            processTouchRotateEvent(motionEvent, motionTracker, i11, motionScene);
            return;
        }
        ...
        if (action == 1) {
            ...
            float[] fArr2 = this.mAnchorDpDt;
            float f13 = f12 != 0.0f ? xVelocity / fArr2[c11] : yVelocity / fArr2[1];
            float f14 = !Float.isNaN(f13) ? (f13 / 3.0f) + progress : progress;
            if (f14 == 0.0f || f14 == 1.0f || (i12 = this.mOnTouchUp) == 3) {
            ...
            this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, f15, f13);
        ...
        if (Math.abs((this.mTouchDirectionX * rawX) + (this.mTouchDirectionY * rawY)) > this.mDragThreshold || this.mDragStarted) {
            ...
            if (i14 != -1) {
                ...
                r24 = 0;
                f11 = progress2;
                this.mMotionLayout.getAnchorDpDt(i14, f11, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
                r23 = 1;
                f11 = progress2;
                r24 = 0;
            ...
            if (Math.abs(((f16 * fArr4[r24]) + (this.mTouchDirectionY * fArr4[r23])) * this.mDragScale) < 0.01d) {
                float[] fArr5 = this.mAnchorDpDt;
                fArr5[r24] = 0.01f;
                fArr5[r23] = 0.01f;
            }
            float max = Math.max(Math.min(f11 + (this.mTouchDirectionX != 0.0f ? rawX / this.mAnchorDpDt[r24] : rawY / this.mAnchorDpDt[r23]), 1.0f), 0.0f);
    ...
    */
    ...
    public void scrollMove(float f11, float f12) {
        float progress = this.mMotionLayout.getProgress();
        if (!this.mDragStarted) {
            this.mDragStarted = true;
        ...
        float max = Math.max(Math.min(progress + (f14 != 0.0f ? (f11 * f14) / this.mAnchorDpDt[0] : (f12 * this.mTouchDirectionY) / this.mAnchorDpDt[1]), 1.0f), 0.0f);
        if (max != this.mMotionLayout.getProgress()) {
            this.mMotionLayout.setProgress(max);
        }
    }

    public void scrollUp(float f11, float f12) {
        int i11;
        this.mDragStarted = false;
        float progress = this.mMotionLayout.getProgress();
        this.mMotionLayout.getAnchorDpDt(this.mTouchAnchorId, progress, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
        float f13 = this.mTouchDirectionX;
        float[] fArr = this.mAnchorDpDt;
        float f14 = f13 != 0.0f ? (f11 * f13) / fArr[0] : (f12 * this.mTouchDirectionY) / fArr[1];
        if (!Float.isNaN(f14)) {
            progress += f14 / 3.0f;
        }
        if (progress == 0.0f || progress == 1.0f || (i11 = this.mOnTouchUp) == 3) {
            return;
        }
        this.mMotionLayout.touchAnimateTo(i11, ((double) progress) >= 0.5d ? 1.0f : 0.0f, f14);
    }
    ...
    public void setDown(float f11, float f12) {
    ...
    public void setUpTouchEvent(float f11, float f12) {
        this.mLastTouchX = f11;
        this.mLastTouchY = f12;
        this.mDragStarted = false;
    }
