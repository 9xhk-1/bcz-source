...
final class UnsafeUtil {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static void logMissingMethod(Throwable e11) {
        Logger.getLogger(UnsafeUtil.class.getName()).log(Level.WARNING, "platform method missing - proto runtime falling back to safer methods: " + e11);
    }
    ...
    public static int mismatch(byte[] left, int leftOff, byte[] right, int rightOff, int length) {
        ...
        int i11 = 0;
        if (HAS_UNSAFE_ARRAY_OPERATIONS) {
            for (int i12 = (BYTE_ARRAY_ALIGNMENT + leftOff) & 7; i11 < length && (i12 & 7) != 0; i12++) {
                if (left[leftOff + i11] != right[rightOff + i11]) {
                    return i11;
                }
                i11++;
            }
            int i13 = ((length - i11) & (-8)) + i11;
            while (i11 < i13) {
                long j11 = BYTE_ARRAY_BASE_OFFSET;
                long j12 = i11;
                long j13 = getLong((Object) left, leftOff + j11 + j12);
                long j14 = getLong((Object) right, j11 + rightOff + j12);
                if (j13 != j14) {
                    return i11 + firstDifferingByteIndexNativeEndian(j13, j14);
                }
                i11 += 8;
            }
        }
    ...
    public static void putByteBigEndian(Object target, long offset, byte value) {
        long j11 = (-4) & offset;
        int i11 = getInt(target, j11);
        int i12 = ((~((int) offset)) & 3) << 3;
        putInt(target, j11, ((255 & value) << i12) | (i11 & (~(255 << i12))));
    }
    ...
    public static void putByteLittleEndian(Object target, long offset, byte value) {
        ...
        putInt(target, j11, ((255 & value) << i11) | (getInt(target, j11) & (~(255 << i11))));
    }
