...
class DefaultDrmSession implements DrmSession {
    ...
    public class RequestHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            try {
                ...
                if (i11 == 1) {
                    th2 = DefaultDrmSession.this.callback.executeProvisionRequest(DefaultDrmSession.this.uuid, (ExoMediaDrm.ProvisionRequest) requestTask.request);
                } else {
                    if (i11 != 2) {
                        throw new RuntimeException();
                    }
                    th2 = DefaultDrmSession.this.callback.executeKeyRequest(DefaultDrmSession.this.uuid, (ExoMediaDrm.KeyRequest) requestTask.request);
                }
            } catch (MediaDrmCallbackException e11) {
                boolean maybeRetryRequest = maybeRetryRequest(message, e11);
                th2 = e11;
                if (maybeRetryRequest) {
                    return;
                }
            } catch (Exception e12) {
                Log.w(DefaultDrmSession.TAG, "Key/provisioning request produced an unexpected exception. Not retrying.", e12);
                th2 = e12;
            }
            ...
            synchronized (this) {
                try {
                    if (!this.isReleased) {
                        DefaultDrmSession.this.responseHandler.obtainMessage(message.what, Pair.create(requestTask.request, th2)).sendToTarget();
                    }
                } finally {
                }
            }
    ...
    private void onError(final Throwable th2, int i11) {
        this.lastException = new DrmSession.DrmSessionException(th2, DrmUtil.getErrorCodeForMediaDrmException(th2, i11));
        Log.e(TAG, "DRM session error", th2);
        if (th2 instanceof Exception) {
            dispatchEvent(new Consumer() { // from class: androidx.media3.exoplayer.drm.d
                @Override // androidx.media3.common.util.Consumer
                public final void accept(Object obj) {
                    ((DrmSessionEventListener.EventDispatcher) obj).drmSessionManagerError((Exception) th2);
                }
            });
        } else {
    ...
    private void onKeysError(Throwable th2, boolean z11) {
    ...
    */
    ...
    private void postKeyRequest(byte[] bArr, int i11, boolean z11) {
        try {
            ...
            ((RequestHandler) Util.castNonNull(this.requestHandler)).post(2, Assertions.checkNotNull(this.currentKeyRequest), z11);
