...
public final class AviExtractor implements Extractor {
    ...
    public class AviSeekMap implements SeekMap {
        ...
        public SeekMap.SeekPoints getSeekPoints(long j11) {
            SeekMap.SeekPoints seekPoints = AviExtractor.this.chunkReaders[0].getSeekPoints(j11);
            for (int i11 = 1; i11 < AviExtractor.this.chunkReaders.length; i11++) {
                SeekMap.SeekPoints seekPoints2 = AviExtractor.this.chunkReaders[i11].getSeekPoints(j11);
    ...
    @Nullable
    private ChunkReader getChunkReader(int i11) {
        for (ChunkReader chunkReader : this.chunkReaders) {
            if (chunkReader.handlesChunkId(i11)) {
                return chunkReader;
            }
        }
        return null;
    ...
    private void parseHdrlBody(ParsableByteArray parsableByteArray) throws IOException {
        ...
        aa<AviChunk> it = parseFrom.children.iterator();
        int i11 = 0;
        while (it.hasNext()) {
            AviChunk next = it.next();
            if (next.getType() == 1819440243) {
                int i12 = i11 + 1;
                ChunkReader processStreamList = processStreamList((ListChunk) next, i11);
    ...
    @Nullable
    private ChunkReader processStreamList(ListChunk listChunk, int i11) {
        ...
        buildUpon.setId(i11);
        int i12 = aviStreamHeaderChunk.suggestedBufferSize;
        if (i12 != 0) {
        ...
        TrackOutput track = this.extractorOutput.track(i11, trackType);
        track.format(buildUpon.build());
        track.durationUs(durationUs);
        this.durationUs = Math.max(this.durationUs, durationUs);
        return new ChunkReader(i11, aviStreamHeaderChunk, track);
    }
    ...
    public int read(ExtractorInput extractorInput, PositionHolder positionHolder) throws IOException {
        ...
        switch (this.state) {
            ...
            case 3:
                ...
                int readLittleEndianInt = this.scratch.readLittleEndianInt();
                int i12 = this.chunkHeaderHolder.chunkType;
                if (i12 == 1179011410) {
                    extractorInput.skipFully(12);
                    return 0;
                }
    ...
    public void seek(long j11, long j12) {
        ...
        this.currentChunkReader = null;
        for (ChunkReader chunkReader : this.chunkReaders) {
            chunkReader.seekToPosition(j11);
        }
        if (j11 != 0) {
            this.state = 6;
    ...
    public AviExtractor(int i11, SubtitleParser.Factory factory) {
        this.subtitleParserFactory = factory;
        this.parseSubtitlesDuringExtraction = (i11 & 1) == 0;
