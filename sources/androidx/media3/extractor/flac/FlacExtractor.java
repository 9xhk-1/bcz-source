...
public final class FlacExtractor implements Extractor {
    ...
    private long findFrame(ParsableByteArray parsableByteArray, boolean z11) {
        ...
        while (position <= parsableByteArray.limit() - this.minFrameSize) {
            ...
            if (parsableByteArray.getPosition() <= parsableByteArray.limit() ? z12 : false) {
                parsableByteArray.setPosition(position);
                return this.sampleNumberHolder.sampleNumber;
            }
            position++;
    ...
    private SeekMap getSeekMap(long j11, long j12) {
        ...
        if (flacStreamMetadata.seekTable != null) {
            return new FlacSeekTableSeekMap(flacStreamMetadata, j11);
        }
        if (j12 == -1 || flacStreamMetadata.totalSamples <= 0) {
            return new SeekMap.Unseekable(flacStreamMetadata.getDurationUs());
        }
        FlacBinarySearchSeeker flacBinarySearchSeeker = new FlacBinarySearchSeeker(flacStreamMetadata, this.frameStartMarker, j11, j12);
