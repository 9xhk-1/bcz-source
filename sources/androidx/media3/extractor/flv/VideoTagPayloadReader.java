...
final class VideoTagPayloadReader extends TagPayloadReader {
    ...
    public boolean parsePayload(ParsableByteArray parsableByteArray, long j11) throws ParserException {
        ...
        if (readUnsignedByte == 0 && !this.hasOutputFormat) {
            ...
            this.output.format(new Format.Builder().setContainerMimeType(MimeTypes.VIDEO_FLV).setSampleMimeType(MimeTypes.VIDEO_H264).setCodecs(parse.codecs).setWidth(parse.width).setHeight(parse.height).setPixelWidthHeightRatio(parse.pixelWidthHeightRatio).setInitializationData(parse.initializationData).build());
        ...
        this.output.sampleMetadata(readInt24, i11, i13, 0, null);
