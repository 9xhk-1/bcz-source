...
class ProfileTranscoder {
    ...
    private static byte[] createCompressibleBody(@NonNull DexProfileData[] dexProfileDataArr, @NonNull byte[] bArr) throws IOException {
        ...
        for (DexProfileData dexProfileData : dexProfileDataArr) {
            i12 += Encoding.utf8Length(generateDexKey(dexProfileData.apkName, dexProfileData.dexName, bArr)) + 16 + (dexProfileData.classSetSize * 2) + dexProfileData.hotMethodRegionSize + getMethodBitmapStorageSize(dexProfileData.numMethodIds);
        }
        ...
        throw Encoding.error("The bytes saved do not match expectation. actual=" + byteArrayOutputStream.size() + " expected=" + i12);
    }
    ...
    private static WritableFileSection createCompressibleClassSection(@NonNull DexProfileData[] dexProfileDataArr) throws IOException {
        ...
        for (int i12 = 0; i12 < dexProfileDataArr.length; i12++) {
            try {
                ...
                Encoding.writeUInt16(byteArrayOutputStream, i12);
                Encoding.writeUInt16(byteArrayOutputStream, dexProfileData.classSetSize);
                i11 = i11 + 4 + (dexProfileData.classSetSize * 2);
        ...
        if (i11 == byteArray.length) {
            WritableFileSection writableFileSection = new WritableFileSection(FileSectionType.CLASSES, i11, byteArray, true);
            byteArrayOutputStream.close();
            return writableFileSection;
        }
        throw Encoding.error("Expected size " + i11 + ", does not match actual size " + byteArray.length);
    }
    ...
    private static WritableFileSection createCompressibleMethodsSection(@NonNull DexProfileData[] dexProfileDataArr) throws IOException {
        ...
        byte[] byteArray = byteArrayOutputStream.toByteArray();
        if (i11 == byteArray.length) {
            WritableFileSection writableFileSection = new WritableFileSection(FileSectionType.METHODS, i11, byteArray, true);
            byteArrayOutputStream.close();
            return writableFileSection;
        }
        throw Encoding.error("Expected size " + i11 + ", does not match actual size " + byteArray.length);
    ...
    private static byte[] createMethodBitmapRegionForS(int i11, @NonNull DexProfileData dexProfileData) throws IOException {
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        try {
            writeMethodBitmapForS(byteArrayOutputStream, i11, dexProfileData);
            byte[] byteArray = byteArrayOutputStream.toByteArray();
            byteArrayOutputStream.close();
            return byteArray;
        } catch (Throwable th2) {
    ...
    private static DexProfileData findByDexName(@NonNull DexProfileData[] dexProfileDataArr, @NonNull String str) {
        ...
        for (int i11 = 0; i11 < dexProfileDataArr.length; i11++) {
            if (dexProfileDataArr[i11].dexName.equals(extractKey)) {
    ...
    private static String generateDexKey(@NonNull String str, @NonNull String str2, @NonNull byte[] bArr) {
        ...
        if (str2.endsWith(UpdateConstants.LOCAL_APK_FILE)) {
            return str2;
        }
    ...
    private static int getMethodBitmapStorageSizeForS(int i11, int i12) {
        return roundUpToByte(Integer.bitCount(i11 & (-2)) * i12) / 8;
    }
    ...
    private static int methodFlagBitmapIndex(int i11, int i12, int i13) {
        ...
        throw Encoding.error("Unexpected flag: " + i11);
    }
    ...
    private static int[] readClasses(@NonNull InputStream inputStream, int i11) throws IOException {
        ...
        for (int i13 = 0; i13 < i11; i13++) {
            i12 += Encoding.readUInt16(inputStream);
            iArr[i13] = i12;
        }
        return iArr;
    }
    ...
    private static int readFlagsFromBitmap(@NonNull BitSet bitSet, int i11, int i12) {
        int i13 = bitSet.get(methodFlagBitmapIndex(2, i11, i12)) ? 2 : 0;
        return bitSet.get(methodFlagBitmapIndex(4, i11, i12)) ? i13 | 4 : i13;
    }
    ...
    private static void readHotMethodRegion(@NonNull InputStream inputStream, @NonNull DexProfileData dexProfileData) throws IOException {
        ...
        while (inputStream.available() > available) {
            ...
            dexProfileData.methods.put(Integer.valueOf(i11), 1);
    ...
    private static DexProfileData[] readMetadataForNBody(@NonNull InputStream inputStream, int i11, DexProfileData[] dexProfileDataArr) throws IOException {
        ...
        for (int i12 = 0; i12 < i11; i12++) {
            ...
            iArr[i12] = Encoding.readUInt16(inputStream);
            strArr[i12] = Encoding.readString(inputStream, readUInt16);
        }
        for (int i13 = 0; i13 < i11; i13++) {
            DexProfileData dexProfileData = dexProfileDataArr[i13];
            if (!dexProfileData.dexName.equals(strArr[i13])) {
                throw Encoding.error("Order of dexfiles in metadata did not match baseline");
            }
            int i14 = iArr[i13];
            dexProfileData.classSetSize = i14;
            dexProfileData.classes = readClasses(inputStream, i14);
        }
    ...
    private static void readMethodBitmap(@NonNull InputStream inputStream, @NonNull DexProfileData dexProfileData) throws IOException {
        ...
        int i11 = 0;
        while (true) {
            int i12 = dexProfileData.numMethodIds;
            if (i11 >= i12) {
                return;
            }
            int readFlagsFromBitmap = readFlagsFromBitmap(valueOf, i11, i12);
            if (readFlagsFromBitmap != 0) {
                Integer num = dexProfileData.methods.get(Integer.valueOf(i11));
                if (num == null) {
                    num = 0;
                }
                dexProfileData.methods.put(Integer.valueOf(i11), Integer.valueOf(readFlagsFromBitmap | num.intValue()));
            }
            i11++;
        }
    }
    ...
    private static DexProfileData[] readUncompressedBody(@NonNull InputStream inputStream, @NonNull String str, int i11) throws IOException {
        ...
        DexProfileData[] dexProfileDataArr = new DexProfileData[i11];
        for (int i12 = 0; i12 < i11; i12++) {
            int readUInt16 = Encoding.readUInt16(inputStream);
            int readUInt162 = Encoding.readUInt16(inputStream);
            dexProfileDataArr[i12] = new DexProfileData(str, Encoding.readString(inputStream, readUInt16), Encoding.readUInt32(inputStream), 0L, readUInt162, (int) Encoding.readUInt32(inputStream), (int) Encoding.readUInt32(inputStream), new int[readUInt162], new TreeMap());
        }
        for (int i13 = 0; i13 < i11; i13++) {
            DexProfileData dexProfileData = dexProfileDataArr[i13];
    ...
    private static void setMethodBitmapBit(@NonNull byte[] bArr, int i11, int i12, @NonNull DexProfileData dexProfileData) {
        int methodFlagBitmapIndex = methodFlagBitmapIndex(i11, i12, dexProfileData.numMethodIds);
    ...
    private static void writeClasses(@NonNull OutputStream outputStream, @NonNull DexProfileData dexProfileData) throws IOException {
        ...
        while (i11 < length) {
            int i13 = iArr[i11];
            Encoding.writeUInt16(outputStream, i13 - i12);
            i11++;
            i12 = i13;
        }
    }
    ...
    private static void writeMethodBitmapForS(@NonNull OutputStream outputStream, int i11, @NonNull DexProfileData dexProfileData) throws IOException {
        byte[] bArr = new byte[getMethodBitmapStorageSizeForS(i11, dexProfileData.numMethodIds)];
        for (Map.Entry<Integer, Integer> entry : dexProfileData.methods.entrySet()) {
            int intValue = entry.getKey().intValue();
            int intValue2 = entry.getValue().intValue();
            int i12 = 0;
            for (int i13 = 1; i13 <= 4; i13 <<= 1) {
                if (i13 != 1 && (i13 & i11) != 0) {
                    if ((i13 & intValue2) == i13) {
                        int i14 = (dexProfileData.numMethodIds * i12) + intValue;
                        int i15 = i14 / 8;
    ...
    private static void writeMethodsWithInlineCaches(@NonNull OutputStream outputStream, @NonNull DexProfileData dexProfileData) throws IOException {
        ...
        for (Map.Entry<Integer, Integer> entry : dexProfileData.methods.entrySet()) {
            ...
            if ((entry.getValue().intValue() & 1) != 0) {
                Encoding.writeUInt16(outputStream, intValue - i11);
    ...
    private static void writeProfileForN(@NonNull OutputStream outputStream, @NonNull DexProfileData[] dexProfileDataArr) throws IOException {
        ...
        for (DexProfileData dexProfileData : dexProfileDataArr) {
            ...
            for (int i11 : dexProfileData.classes) {
                Encoding.writeUInt16(outputStream, i11);
            }
        }
    }
    ...
    private static void writeProfileSections(@NonNull OutputStream outputStream, @NonNull DexProfileData[] dexProfileDataArr) throws IOException {
        ...
        for (int i11 = 0; i11 < arrayList.size(); i11++) {
            WritableFileSection writableFileSection = (WritableFileSection) arrayList.get(i11);
