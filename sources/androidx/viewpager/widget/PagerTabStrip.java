...
public class PagerTabStrip extends PagerTitleStrip {
    ...
    public void onDraw(Canvas canvas) {
        ...
        float f11 = height;
        canvas.drawRect(left, i11, right, f11, this.mTabPaint);
        if (this.mDrawFullUnderline) {
            this.mTabPaint.setColor((this.mIndicatorColor & 16777215) | (-16777216));
            canvas.drawRect(getPaddingLeft(), height - this.mFullUnderlineHeight, getWidth() - getPaddingRight(), f11, this.mTabPaint);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            if (action == 2 && (Math.abs(x11 - this.mInitialMotionX) > this.mTouchSlop || Math.abs(y11 - this.mInitialMotionY) > this.mTouchSlop)) {
                this.mIgnoreTap = true;
            }
        } else if (x11 < this.mCurrText.getLeft() - this.mTabPadding) {
            ViewPager viewPager = this.mPager;
            viewPager.setCurrentItem(viewPager.getCurrentItem() - 1);
        } else if (x11 > this.mCurrText.getRight() + this.mTabPadding) {
    ...
    public void setBackgroundColor(@ColorInt int i11) {
        ...
        this.mDrawFullUnderline = (i11 & (-16777216)) == 0;
    }
