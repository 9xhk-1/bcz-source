...
public class AsyncRequestQueue extends RequestQueue {
        ...
        private ExecutorFactory getDefaultExecutorFactory() {
            return new ExecutorFactory() { // from class: com.android.volley.AsyncRequestQueue.Builder.1
                private ThreadPoolExecutor getNewThreadPoolExecutor(int i11, String str, BlockingQueue<Runnable> blockingQueue) {
                    return new ThreadPoolExecutor(0, i11, 60L, TimeUnit.SECONDS, blockingQueue, getThreadFactory(str));
                }
    ...
    public class CacheParseTask<T> extends RequestTask<T> {
        ...
        public CacheParseTask(Request<T> request, Cache.Entry entry, long j11) {
            super(request);
            this.entry = entry;
    ...
    public class NetworkParseTask<T> extends RequestTask<T> {
        ...
        public void run() {
            ...
            if (!this.mRequest.shouldCache() || parseNetworkResponse.cacheEntry == null) {
                ...
                AsyncRequestQueue.this.mNonBlockingExecutor.execute(AsyncRequestQueue.this.new CachePutTask(this.mRequest, parseNetworkResponse));
            } else {
                AsyncRequestQueue.this.mBlockingExecutor.execute(AsyncRequestQueue.this.new CachePutTask(this.mRequest, parseNetworkResponse));
            }
        }
    }
