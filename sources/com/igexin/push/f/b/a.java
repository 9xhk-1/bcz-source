package com.igexin.push.f.b;

import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.TimeUnit;

/* loaded from: classes7.dex */
public class a extends f {

    /* renamed from: b, reason: collision with root package name */
    private static volatile a f38558b;

    /* renamed from: a, reason: collision with root package name */
    private List<c> f38559a;

    private a() {
        super(60000L, (byte) 0);
        this.f37227p = true;
        this.f38559a = new ArrayList();
    }

    public static a g() {
        if (f38558b == null) {
            synchronized (a.class) {
                try {
                    if (f38558b == null) {
                        f38558b = new a();
                    }
                } finally {
                }
            }
        }
        return f38558b;
    }

    private void i() {
        a(360000L, TimeUnit.MILLISECONDS);
    }

    public final boolean a(c cVar) {
        List<c> list = this.f38559a;
        return (list == null || list.contains(cVar) || !this.f38559a.add(cVar)) ? false : true;
    }

    @Override // com.igexin.c.a.d.a.e
    public final int c() {
        return 0;
    }

    @Override // com.igexin.push.f.b.f
    public final void h() {
        com.igexin.push.core.a.b.d();
        com.igexin.push.core.a.b.k();
        for (c cVar : this.f38559a) {
            if (cVar.c()) {
                cVar.b();
                cVar.a(System.currentTimeMillis());
            }
        }
        a(360000L, TimeUnit.MILLISECONDS);
        com.igexin.c.a.b.e.a().a((Object) this);
    }
}
