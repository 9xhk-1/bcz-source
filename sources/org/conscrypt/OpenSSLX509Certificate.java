...
public final class OpenSSLX509Certificate extends X509Certificate {
    ...
    public static List<OpenSSLX509Certificate> fromPkcs7DerInputStream(InputStream inputStream) throws OpenSSLX509CertificateFactory.ParsingException {
        ...
        try {
            try {
                ...
                for (long j11 : d2i_PKCS7_bio) {
                    if (j11 != 0) {
                        arrayList.add(new OpenSSLX509Certificate(j11));
                    }
                }
                return arrayList;
            } catch (Exception e11) {
                throw new OpenSSLX509CertificateFactory.ParsingException(e11);
            }
        } finally {
    ...
    public static List<OpenSSLX509Certificate> fromPkcs7PemInputStream(InputStream inputStream) throws OpenSSLX509CertificateFactory.ParsingException {
        ...
        try {
            try {
                ...
                for (long j11 : PEM_read_bio_PKCS7) {
                    if (j11 != 0) {
                        arrayList.add(new OpenSSLX509Certificate(j11));
                    }
                }
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (getNotBefore().compareTo(date) > 0) {
            throw new CertificateNotYetValidException("Certificate not valid until " + getNotBefore().toString() + " (compared to " + date.toString() + j.f81007d);
        }
        if (getNotAfter().compareTo(date) >= 0) {
            return;
        }
        throw new CertificateExpiredException("Certificate expired at " + getNotAfter().toString() + " (compared to " + date.toString() + j.f81007d);
    }
