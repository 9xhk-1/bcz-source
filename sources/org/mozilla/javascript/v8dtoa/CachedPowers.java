...
public class CachedPowers {
    ...
    public static class CachedPower {
        ...
        public CachedPower(long j11, short s11, short s12) {
            this.significand = j11;
            this.binaryExponent = s11;
            this.decimalExponent = s12;
        }
    }

    public static int getCachedPower(int i11, int i12, int i13, DiyFp diyFp) {
        CachedPower cachedPower = CACHED_POWERS[((((int) Math.ceil(((i12 - i11) + 63) * kD_1_LOG2_10)) + 307) / 8) + 1];
        diyFp.setF(cachedPower.significand);
        diyFp.setE(cachedPower.binaryExponent);
