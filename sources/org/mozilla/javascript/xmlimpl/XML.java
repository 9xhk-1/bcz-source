...
class XML extends XMLObjectImpl {
    ...
    private String ecmaToString() {
        ...
        for (int i11 = 0; i11 < this.node.getChildCount(); i11++) {
            XmlNode child = this.node.getChild(i11);
    ...
    private int getChildIndexOf(XML xml) {
        for (int i11 = 0; i11 < this.node.getChildCount(); i11++) {
            if (this.node.getChild(i11).isSameNode(xml.node)) {
                return i11;
            }
        }
        return -1;
    ...
    private XmlNode[] getNodesForInsert(Object obj) {
        ...
        for (int i11 = 0; i11 < xMLList.length(); i11++) {
            xmlNodeArr[i11] = xMLList.item(i11).node;
        }
        return xmlNodeArr;
    }
    ...
    public XMLList child(XMLName xMLName) {
        ...
        for (int i11 = 0; i11 < matchingChildren.length; i11++) {
            if (xMLName.matchesElement(matchingChildren[i11].getQname())) {
                newXMLList.addToList(toXML(matchingChildren[i11]));
            }
        }
    ...
    public void deleteXMLProperty(XMLName xMLName) {
        ...
        for (int i11 = 0; i11 < propertyList.length(); i11++) {
            propertyList.item(i11).node.deleteMe();
        }
    }
    ...
    public XMLList elements(XMLName xMLName) {
        ...
        XmlNode[] matchingChildren = this.node.getMatchingChildren(XmlNode.Filter.ELEMENT);
        for (int i11 = 0; i11 < matchingChildren.length; i11++) {
            if (xMLName.matches(toXML(matchingChildren[i11]))) {
                newXMLList.addToList(toXML(matchingChildren[i11]));
            }
        }
        return newXMLList;
    ...
    public XML[] getAttributes() {
        ...
        for (int i11 = 0; i11 < length; i11++) {
            xmlArr[i11] = toXML(attributes[i11]);
        }
        return xmlArr;
    }
    ...
    public XML[] getChildren() {
        ...
        XML[] xmlArr = new XML[length];
        for (int i11 = 0; i11 < length; i11++) {
            xmlArr[i11] = toXML(matchingChildren[i11]);
        }
        return xmlArr;
    }
    ...
    public XML getXmlChild(int i11) {
        XmlNode child = this.node.getChild(i11);
    ...
    public Object jsConstructor(Context context, boolean z11, Object[] objArr) {
        ...
        return z11 ? ecmaToXml.copy() : ecmaToXml;
    }
    ...
    public XML replace(int i11, Object obj) {
        XMLList child = child(i11);
        if (child.length() > 0) {
    ...
    public XMLList child(int i11) {
        ...
        if (i11 >= 0 && i11 < this.node.getChildCount()) {
            newXMLList.addToList(getXmlChild(i11));
        }
        return newXMLList;
    }
